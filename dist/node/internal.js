...
module.exports = {
  ...
  addFiles: function(zip, htmlSource, documentOptions) {
    ...
    zip.folder('word').file('document.xml', this.renderDocumentFile(documentOptions)).file('afchunk.mht', utils.getMHTdocument(htmlSource[0])).file('afchunk1.mht', utils.getMHTdocument(htmlSource[1])).file('endnotes.xml', fs.readFileSync(__dirname + '/templateFile/word/endnotes.xml')).file('fontTable.xml', fs.readFileSync(__dirname + '/templateFile/word/fontTable.xml')).file('footer1.xml', this.renderFooter(documentOptions)).file('footnotes.xml', fs.readFileSync(__dirname + '/templateFile/word/footnotes.xml')).file('settings.xml', (fs.readFileSync(__dirname + '/templateFile/word/settings.xml', 'utf-8')).replace(/w:val="true"/g, documentOptions.config.content ? 'w:val="true"' : 'w:val="false"')).file('styles.xml', fs.readFileSync(__dirname + '/templateFile/word/styles.xml')).file('webSetting.xml', fs.readFileSync(__dirname + '/templateFile/word/webSettings.xml')).folder('_rels').file('document.xml.rels', fs.readFileSync(__dirname + '/templateFile/word/_rels/document.xml.rels'));
